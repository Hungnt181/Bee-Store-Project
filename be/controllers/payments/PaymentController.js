...
export const add = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Thêm phương thức thanh toán thành công", data });
  } catch (error) {
...
export const remove = async (req, res) => {
  try {
    ...
    if (!data) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy phương thức thanh toán để xóa" });
    }
    res
      .status(200)
      .json({ message: "Xóa phương thức thanh toán thành công", data });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
};
...
export const edit = async (req, res) => {
  try {
    ...
    if (!data) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy phương thức thanh toán để cập nhật" });
    }

    res
      .status(200)
      .json({ message: "Cập nhật phương thức thanh toán thành công", data });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
};
