...
export default function FilterSide({ params, setParams }: FilterProps) {
  ...
  const handleCateChange = useCallback((id: string) => {
  ...
  useEffect(() => {
    const getCateInParams = searchParams.get("cate")
    if (getCateInParams && getCateInParams === "all") {
      setSizeChange([]);
      setColorChange([]);
      setCateChange([]);
    ...
    if (getCateInParams && getCateInParams !== "all") {
      setCateChange(() => {
        const newCateChange = [getCateInParams];
        setParams({ cate: newCateChange });
        return newCateChange;
      });
      setSizeChange([]);
      setColorChange([]);
        ...
        <ul className="flex flex-col gap-2 py-1">
          {listCate?.data?.map((item: Category, index: number) => {
            ...
            return (
              <li
                key={index}
                className={`transition-colors duration-200 rounded-md px-2 py-1 ${isChecked ? "bg-gray-200" : "hover:bg-gray-50"
                  }`}
              >
                <Checkbox
                  value={item?._id}
