...
const VoucherEditPage = () => {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ["VOUCHER", id],
        queryFn: async () => {
            const { data } = await axios.get(`http://localhost:3000/api/vouchers/${id}`);
            return data.data;
        },
    });
  ...
  const errorMsg =
    (error.response?.data as { message?: string })?.message || 'Lỗi khi cập nhật voucher';
...
,
    ...
    return (
        ...
        >
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16 }}>
                <EditOutlined style={{ fontSize: 24, marginRight: 12, color: '#1677ff' }} />
                        ...
                        <Card 
                            ...
                            extra={
                                <Badge 
                                    ...
                                    text={data?.status ? "Đang hoạt động" : "Không hoạt động"} 
                                />
                            }
                        >
                            <Row gutter={16}>
  ...
  return Number(cleaned);
                    ...
                    <Col span={24}>
                        <Card 
                            title={<Text strong>Trạng thái hoạt động</Text>} 
                            size="small" 
                            headStyle={{ backgroundColor: '#f5f5f5' }}
                            style={{ marginBottom: 24 }}
                        >
                            <Form.Item 
                                ...
                                tooltip="Bật/tắt để kích hoạt hoặc vô hiệu hóa voucher này"
                            >
                                <Switch 
